				...
				if (keypress == INS)
				{
					...
					if (i == 0)
					{
						...
						else if ((j == 0 || j == 3) && k == 1)
						{
							...
							if (j == 0)
								strcpy(thongBaoKhongDu[5], thongBaoHang.c_str());
							char choiceKhongDu[2][15] = { "Nhap lai","Thoat" };
							if (j == 3)
								strcpy(thongBaoKhongDu[3], "Ma Vat Tu Khong Ton Tai");
...
void DSCTHoaDon::FindVatTu(ChiTietVatTu & newVatTu)
{
	if (keypress == CTRL_F)
	{
		...
		char choice[2][15] = { "Them","Thoat" };
		if (temp != NULL)
			strcpy(newVatTu.maVT, temp->maVT);
...
void DSCTHoaDon::InputAdd(ChiTietVatTu & newVatTu)
{
	...
	case DSCTHoaDon::themMa:
	{
		...
		strcpy(newVatTu.maVT, temp.c_str());
		break;
	}
...
PTRTop DSCTHoaDon::TinhTopDoanhThu(Ngay start, Ngay end)
{
	...
	for (int dsNV = 0; dsNV < 3; dsNV++)
	{
		...
		for (int i = 0; i < dsTemp.GetDanhSachNhanVien().soLuongNhanVien; i++)
		{
			...
			{
				if (DateCmp(end, p->infoHoaDon.ngayNhapHoaDon) >= 0 && DateCmp(p->infoHoaDon.ngayNhapHoaDon, start) >= 0)
...
void DSCTHoaDon::DoanhThu(Ngay startDay, Ngay endDay)
{
	if (keypress == INS)
	{
		...
		{
			for (PTRTop temp = this->firstRank; temp != NULL; temp = temp->right)
			{
				this->lastRank = temp;
				if (stop < 10)
					this->lastListRank = temp;
				if (stop == rankAmount)
					break;
			}
			this->DrawDoanhThu();
